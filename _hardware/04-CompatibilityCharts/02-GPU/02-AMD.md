...
{: .note }
Not much to say, this card has native support OOB without the need for kexts.
...
{: .note }
WhateverGreen is recommended for these GPUs.
...
{: .note }
Vega 10 GPUs are natively supported in macOS. The only brand of GPUs to avoid with Vega 10 is XFX, due to VBIOS communication issues, which can't be easily solved with a reference BIOS as the Vega's powerplay table interacts with the OS and GPU differently.
